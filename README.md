...
- **Price alerts** with toast notifications when targets are hit
- **Responsive layout** — desktop sidebar + mobile hamburger menu
- **Mock authentication** with secure HttpOnly cookie JWT
...
- **ws over socket.io**: Lighter, no polling fallback needed for a modern app
- **Vite over Next.js**: Client-side real-time app, no SSR needed
- **Zustand over Redux**: Minimal boilerplate for small state surface
- **In-memory cache over Redis**: Single-service, no external dependencies
